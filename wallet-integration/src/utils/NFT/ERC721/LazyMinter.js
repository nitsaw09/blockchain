...
const SIGNING_DOMAIN_NAME = "GEM-NFT";
...
class LazyMinter {
  ...
  /**
   * Creates a new Auth object and signs it using this LazyMinter's signing key.
   *
   ...
   * @returns {Auth}
   */
  async createAuth(message, nonce) {
    const login = { message, nonce };
    ...
    const signature = await this.signer._signTypedData(domain, types, login);
    return {
      ...login,
      signature
    };
